...
* **Test003.in:** Đánh trúng điểm yếu của thuật toán MSD Radix Sort. Việc chia nhánh quá rộng ở 4 ký tự đầu tạo ra hàng nghìn "bucket" (xô) trống rỗng. Thuật toán vẫn phải cấp phát bộ nhớ và gọi đệ quy liên tục cho các bucket chỉ chứa 1 phần tử, tạo ra lượng hao phí tài nguyên (cache miss và overhead) khổng lồ.
* **Test004.in:** Tấn công trực diện vào những lập trình viên chọn pivot cho Quick Sort bằng phần tử đầu, cuối hoặc giữa mà quên bước ngẫu nhiên hóa (randomize). Với mảng sắp xếp ngược, Quick Sort sẽ luôn chia mảng thành một bên $0$ phần tử và một bên $N-1$ phần tử, lập tức đẩy thời gian chạy lên $O(N^2)$.
